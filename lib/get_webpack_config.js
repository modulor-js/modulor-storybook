...
const getWebpackConfig = () => customFs.checkFile(webpackConfigPath).then((exists) => {
  !exists && console.log(`${webpackConfigPath} does'n exist. Using default config`);
  return merge(
    require('../defaults/webpack.config.js'),
    exists ? require(webpackConfigPath) : {}
  );
});
